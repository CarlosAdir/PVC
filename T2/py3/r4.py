...
pasta_parametros_intrinsicos = "../data/img_r2/"
...
if __name__ == "__main__":
	...
	for pasta in pastas:
		...
		if n_pack == 0: # Ou seja, nao tem nenhum pacote
			...
			intr_med, dist_med = use.get_statistics_matrices(pasta_parametros_intrinsicos, n_pack, n_img, verbose = False) # The intrisic parameters
			newcameramtx, roi = cv2.getOptimalNewCameraMatrix(intr_med,dist_med,(use.W,use.H),1,(use.W,use.H))

			rvec, T = use.get_extrinsic_parameters(pasta, n_pack, n_img) # The extrinsic parameters
			...
			while(True):					# The main loop where the raw image and undistorted img are
				...
				mapx,mapy = cv2.initUndistortRectifyMap(intr_med,dist_med,R,newcameramtx,(use.W,use.H),5)
